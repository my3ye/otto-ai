...
def _build_system_prompt(context_text: str, channel: str, is_voice: bool = False,
    ...
    return (
        ...
        f"The LAST user message is Mev's CURRENT message — respond ONLY to that message. "
        f"Prior turns are context only; do NOT address or answer old topics unless Mev's "
        f"current message explicitly references them. Rules:\n"
        f"1. Respond to the LATEST message only. Old topics are resolved — don't resurface them.\n"
        f"2. NEVER repeat information Mev has already confirmed, acknowledged, or told you.\n"
        f"3. If Mev said 'done', 'already did that', 'all set', 'yes', or confirmed a fact — "
        f"treat it as ESTABLISHED. Do not re-explain, re-ask, or re-suggest it.\n"
        f"4. If you asked a question and Mev answered it, do not ask again.\n"
        f"5. If Mev corrected you, accept the correction — do not restate your original claim.\n"
        f"{confirmed_block}\n\n"
