...
- [Custom Chain / L2 / L3 / Appchain Build Options](project_custom_chain_appchain_2026_04_13.md) — PIPELINE COMPLETE 7.5/10 (post-corrections). Base=primary (Arbitrum One secondary in zkPresence). Zero appchain code in all repos (grep-verified). 10 memories + 1 note stored (3064e558). 7 patches: Insights 2,3,7 HIGH→MEDIUM (vendor-biased/single sources); Insight 1 "locked"→"primary"; pricing date-qualified Mar 2026; Action 3 Aztec non-sequitur removed; Base profitability softened. P0: ONEON ZK Stack L3 RaaS Q3 2026 target. P1: SOS Cosmos Phase 2 spec post-PMF. P2: ZK Stack grant / Optimism retroPGF. DOC RISK: invisible-web3-layer-architecture-2026-03-28.md not git-tracked.

- [Sui & Aptos Move-based chains 2026](project_sui_aptos_move_2026_04_13.md) — PIPELINE COMPLETE 7.5/10 (post-corrections). Sui=consumer/identity (954 devs, zkLogin), Aptos=institutional/RWA ($540M+, BlackRock, SEC commodity). Zero Move code in any Otto project (grep-verified). 9 memories stored. 4 patches: (1) CRITICAL — Action 1 reframed: zkLogin *pattern* in SP1/Base, NOT @mysten/zklogin SDK; (2) Insight 2 HIGH→MEDIUM (Electric Capital 1 source); (3) Insight 5 HIGH→MEDIUM (chainspect single source); (4) Insight 6 HIGH→MEDIUM (2 internal sources). EVM-first (Base) unchanged. P0: zkLogin pattern in SP1 circuit. P1: Aptos RWA editorial (Oct 2026 unlock). P2: CME SUI futures May 4 content window.
